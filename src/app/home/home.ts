...
export class Home implements OnInit, OnDestroy, AfterViewInit {
  ...
  carouselImages = [
    '/assets/V1.jpg','/assets/V5.PNG','/assets/V10.PNG','/assets/V9.PNG',
    '/assets/V12.jpg','/assets/V13.jpg','/assets/V14.jpg','/assets/V18.PNG',
    '/assets/V19.PNG','/assets/V20.PNG','/assets/V21.PNG','/assets/V22.PNG'
  ];
  ...
  get maisonElectroJeuxProducts() {
    return this.categories
      .filter(c => c.name === 'Maison' || c.name === 'Électroniques' || c.name === 'Jeux')
      .flatMap(c => c.products);
  }
  ...
  expertiseCards = ['Vendeurs Vérifiés','Paiement Sécurisé','Livraison Rapide','Produits de Qualité','Support 24/7'];
  private coverInterval!: any;

  ngOnInit() {
    setTimeout(() => this.heroAnimated = true, 100);
    this.coverInterval = setInterval(() => this.rotateCoverflow(), 2500);
  }

  ngAfterViewInit(): void {
    if (this.isBrowser) {
      this.ngZone.runOutsideAngular(() => setTimeout(() => this.initScrollAnimations(), 200));
    }
  }
  ...
  private initScrollAnimations(): void {
    ...
    gsap.utils.toArray('.products-section').forEach((section: any) => {
      ...
      if (heading) gsap.fromTo(heading, { opacity: 0, x: -40 }, {
        opacity: 1, x: 0, duration: 0.7, ease: 'power2.out',
        scrollTrigger: { trigger: section, start: 'top 80%', toggleActions: 'play none none none' }
      });

      if (scrollContainer) gsap.fromTo(scrollContainer, { opacity: 0, y: 30 }, {
    ...
    if (vendeurSection) {
      ...
      if (leftContent) gsap.fromTo(leftContent, { opacity: 0, x: -60 }, {
        opacity: 1, x: 0, duration: 1, ease: 'power3.out',
        scrollTrigger: { trigger: vendeurSection, start: 'top 70%', toggleActions: 'play none none none' }
      });

      if (rightContent) gsap.fromTo(rightContent, { opacity: 0, x: 60 }, {
    ...
    gsap.utils.toArray('.why-card').forEach((card: any, index: number) => {
      gsap.fromTo(card, { opacity: 0, y: 50, scale: 0.9 }, {
        opacity: 1, y: 0, scale: 1, duration: 0.6, delay: index * 0.1, ease: 'back.out(1.4)',
        scrollTrigger: { trigger: card, start: 'top 85%', toggleActions: 'play none none none' }
      });
    });
  ...
  rotateCoverflow() {
    this.expertiseCards = [...this.expertiseCards.slice(1), this.expertiseCards[0]];
  }

  ngOnDestroy() {
    clearInterval(this.coverInterval);
    if (this.isBrowser) ScrollTrigger.getAll().forEach((st: ScrollTrigger) => st.kill());
  }
}
